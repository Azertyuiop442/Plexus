...
fn vendor_path(vendor: &str) -> PathBuf {
    let p = skills_root().join(vendor);
    if p.exists() {
        return p;
    }
    let p_agents = agents_skills_root().join(vendor);
    if p_agents.exists() {
        return p_agents;
    }
    p
...
pub fn discover_vendors() -> Vec<String> {
    ...
    for root in [skills_root(), agents_skills_root()] {
...
fn vendor_has_skill(vendor_dir: &Path) -> bool {
    if vendor_dir.join("SKILL.md").is_file() {
        return true;
    }
...
pub fn discover_skills(vendor: &str) -> Vec<SkillEntry> {
    ...
    if root.join("SKILL.md").is_file() {
        let extra = skill_files_in(&root);
        return vec![SkillEntry {
            name: vendor.to_string(),
            skill_md: root.join("SKILL.md"),
            extra_files: extra,
        }];
    }
    let entries = match fs::read_dir(&root) {
...
pub fn vendor_status(name: &str) -> VendorStatus {
    ...
    if status.url.is_none() {
        status.url = crate::skills_cli::read_skill_lock_url(name);
    }
...
pub fn update_all_behind_async(events: MuxEventSender) {
    ...
    std::thread::spawn(move || {
        ...
        let _ = events.send(MuxEvent::SkillsUpdateDone { ok, failed, summary: None });
    });
}

pub use crate::skills_cli::run_skills_cli_update;

pub fn install_skill(input: &str, vendor_override: Option<&str>) -> Result<(String, usize), String> {
    let clean = crate::skills_cli::parse_skill_input(input);
    if clean.is_empty() {
        return Err("Skill target is empty".into());
    }

    if let Ok(res) = crate::skills_cli::install_via_skills_cli(&clean) {
        return Ok(res);
    }

    install_skills_bundle(&clean, vendor_override)
}
...
mod tests {
    ...
    fn discover_vendors_returns_folders_containing_skill_md() {
        ...
        fs::write(skills.join("gamma/SKILL.md"), "x").unwrap();
        let names = discover_vendors();
    ...
    #[test]
    fn discover_skills_returns_flat_skill_entry() {
        let _g = HOME_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let h = fresh_home("discover-flat-skill");
        let folder = h.join(".commandcode/skills/craft");
        fs::create_dir_all(&folder).unwrap();
        fs::write(folder.join("SKILL.md"), "content").unwrap();
        let skills = discover_skills("craft");
        assert_eq!(skills.len(), 1);
        assert_eq!(skills[0].name, "craft");
        assert!(skills[0].skill_md.ends_with("craft/SKILL.md"));
    }
