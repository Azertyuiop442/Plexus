...
fn handle_skills_back(state: &mut AppState) {
    crate::ui::modal::skills::handle_skills_back(state);
}

fn handle_skills_browse(state: &mut AppState, key: &str) {
    crate::ui::modal::skills::handle_skills_browse(state, key);
}
...
fn handle_skills_tracker(state: &mut AppState, key: &str) {
    ...
    } else if key == "skills.cli_update" {
        let events = state.events.clone();
        state.skills_view.last_update_summary = Some("Checking updates via skills CLI...".into());
        crate::ui::modal::open_skills_modal(state);
        state.dirty = true;
        std::thread::spawn(move || {
            let res = crate::skills::run_skills_cli_update();
            let (ok, failed, msg) = match res {
                Ok(out) => (1, 0, out),
                Err(err) => (0, 1, format!("✗ {err}")),
            };
            let _ = events.send(crate::mux_events::MuxEvent::SkillsUpdateDone {
                ok,
                failed,
                summary: Some(msg),
            });
...
fn handle_skills_sources(state: &mut AppState, key: &str) {
    if key == "source.back" {
        state.skills_view.selected_vendor = None;
        crate::ui::modal::open_skills_modal(state);
        return;
    }
    if let Some(vendor) = key.strip_prefix("source.open.") {
        state.skills_view.selected_vendor = Some(vendor.to_string());
        crate::ui::modal::open_skills_modal(state);
        return;
    }
    if let Some(vendor) = key.strip_prefix("source.update.") {
        let events = state.events.clone();
        let vendor_name = vendor.to_string();
        std::thread::spawn(move || {
            let res = crate::skills::pull_vendor(&vendor_name);
            let _ = events.send(crate::mux_events::MuxEvent::SkillsUpdateDone {
                ok: if res.is_ok() { 1 } else { 0 },
                failed: if res.is_err() { 1 } else { 0 },
                summary: match res {
                    Ok(m) => Some(m),
                    Err(e) => Some(format!("✗ {e}")),
                },
    ...
    if let Some(vendor) = key.strip_prefix("source.save.") {
        if let Some(modal) = state.active_modal.as_mut() {
            for row in &modal.rows {
                if let crate::ui::modal::model::ModalRow::TextInput { key: row_k, value, .. } = row {
                    if let Some(v_name) = row_k.strip_prefix("url.") {
                        if v_name == vendor {
        ...
        crate::skills::check_all_background(state.events.clone());
        state.skills_view.last_update_summary = Some(format!("✓ Saved remote URL for {vendor}"));
        crate::ui::modal::open_skills_modal(state);
        return;
    }
    if let Some(modal) = state.active_modal.as_mut() {
        ...
        } else if modal.rows.iter().any(|r| matches!(r, crate::ui::modal::model::ModalRow::TextInput { .. })) {
...
fn handle_skills_install(state: &mut AppState, key: &str) {
    ...
    match crate::skills::install_skill(&url, vendor_opt) {
...
pub(crate) fn write_pickup_and_close(state: &mut AppState) {
    let path = match (
...
pub fn handle_key(state: &mut AppState, key: KeyEvent, command: &str, new_tab_cmd: &str) {
    ...
    if key.code == KeyCode::Esc {
        ...
        if crate::ui::modal::skills::is_skills_modal(state) {
            if state.skills_view.pending_delete.is_some() {
                state.skills_view.pending_delete = None;
                crate::ui::modal::open_skills_modal(state);
                return;
            }
            if state.skills_view.show_detail.is_some() {
                state.skills_view.show_detail = None;
                crate::ui::modal::open_skills_modal(state);
                return;
            }
            if state.skills_view.selected_family.is_some() {
                state.skills_view.selected_family = None;
                crate::ui::modal::open_skills_modal(state);
                return;
            }
            if state.skills_view.selected_vendor.is_some() {
                state.skills_view.selected_vendor = None;
                crate::ui::modal::open_skills_modal(state);
                return;
            }
            if !state.skills_view.path.is_empty() {
            ...
            KeyCode::Char(c) => {
                ...
                if let Some(ModalRow::TextInput { value, .. }) = modal.rows.get_mut(idx) {
                    ...
                    match c {
                        ...
                        'v' => {
                            crate::ui::modal::skills::handle_skills_vendor_toggle_shortcut(state);
                        }
                        'i' | 'd' => {
                            crate::ui::modal::skills::handle_skills_info_shortcut(state);
                        }
                        'a' => {
                            crate::ui::modal::skills::handle_skills_all_shortcut(state, true);
                        }
                        'n' => {
                            crate::ui::modal::skills::handle_skills_all_shortcut(state, false);
                        }
                        _ => {}
                    }
                }
