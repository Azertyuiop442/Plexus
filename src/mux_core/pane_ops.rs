...
pub fn spawn_pane(state: &mut AppState, cmd: &str, cols: u16, rows: u16) -> io::Result<()> {
    ...
    std::thread::spawn(move || {
        ...
        crate::orphan_journal::reap_child(child_pid);
    ...
    if child_pid != 0 {
        ...
        std::thread::spawn(move || {
            loop {
                ...
                let alive = crate::orphan_journal::is_process_alive(child_pid);
                if !alive {
    ...
    if state.panes.len() > 1 {
        ...
        if let Some(mut logo) = logo {
            if let Some(last) = state.panes.last() {
                let mut new_pane = last.lock().unwrap_or_else(|e| e.into_inner());
