...
pub struct AppState {
    ...
    pub resizing_panel_dragged: bool,

    pub resizing_sidebar: bool,

    pub resizing_sidebar_dragged: bool,
...
impl AppState {
    pub fn new(sidebar: Sidebar) -> Self {
        ...
        let mut state = Self {
            ...
            resizing_panel_dragged: false,
            resizing_sidebar: false,
            resizing_sidebar_dragged: false,
    ...
    pub fn sync_sidebar_panel(&mut self) {
        self.sidebar.active_panel = if self.panel_active {
            Some(self.active_right_sidebar)
        } else {
            None
        };
        self.sidebar.panel_open = self.panel_active && self.panel_sidebar_open;
    }
    ...
    pub fn refresh_model_cache(&mut self) {
        ...
        let session_id = self
            ...
            .and_then(|g| g.state.session_id.clone());
        let project = self.sidebar.project.as_str();
        let live_model = session_id
            .as_deref()
            .and_then(|sid| crate::ui::session_meta::read_session_model(project, sid));
        let model = live_model.or_else(|| self.mods_data.active_model());
    ...
    pub fn refresh_mods_now(&mut self) {
        ...
        let new_panels: Vec<_> = self
            ...
            .collect();
        if self.sidebar.panels != new_panels {
            self.sidebar.panels = new_panels;
            self.sidebar.rebuild_rows();
            self.dirty = true;
        }
