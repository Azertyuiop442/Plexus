...
use crate::dashboard_meta::{
    beat_dashboard_meta, now_ms, remove_dashboard_meta_if_ours, write_dashboard_meta,
    DASHBOARD_BEAT_MS,
};
...
fn main() -> io::Result<()> {
    ...
    print!("\x1b[48;2;0;0;0m\x1b[2J\x1b[H");
    {
    ...
    let left_w = if state.sidebar_open { state.sidebar_w } else { 1 };
    let cols = size.width.saturating_sub(left_w + 3).max(20);
    ...
    let mut last_dashboard_beat = std::time::Instant::now();
    ...
    let mut dropped_output_gens = std::collections::HashSet::<u64>::new();
    let mut unknown_exit_gens = std::collections::HashSet::<u64>::new();
    ...
    loop {
        ...
        while let Ok(ev) = event_rx.try_recv() {
            match ev {
                crate::mux_events::MuxEvent::PtyOutput { gen, bytes } => {
                    ...
                    match find_pane_by_gen(&state, gen) {
                        Some(pane) => {
                        ...
                        None => {
                            if dropped_output_gens.insert(gen) {
                                crate::ipc::log_append(
                                    "resize.log",
                                    &format!("pty_output_dropped: gen={gen}"),
                                );
                            }
                        }
                    }
                ...
                crate::mux_events::MuxEvent::PaneExited { gen } => {
                    match find_pane_by_gen(&state, gen) {
                        ...
                        None => {
                            if unknown_exit_gens.insert(gen) {
                                crate::ipc::log_append(
                                    "resize.log",
                                    &format!("pane_exit_unknown: gen={gen}"),
                                );
                            }
                        }
                    }
                ...
                crate::mux_events::MuxEvent::SkillsUpdateDone { ok, failed, summary } => {
                    state.skills_view.updating = None;
                    state.skills_view.last_update_summary =
                        summary.or_else(|| Some(format!("{ok} updated, {failed} failed")));
                    crate::skills::check_all_background(state.events.clone());
                    if crate::ui::modal::skills::is_skills_modal(&state) {
                        crate::ui::modal::open_skills_modal(&mut state);
                    }
                                ...
                                if let Some(modals) = json.get("modals").and_then(|m| m.as_array())
                                {
                                    for modal in modals {
                                        if modal.get("pending").and_then(|p| p.as_bool())
                                            == Some(true)
                                        {
                                            let Some(active) = state.active_modal.as_mut() else {
                                                continue;
                                            };
        ...
        if last_sidebar_refresh.elapsed() >= std::time::Duration::from_secs(5) {
            ...
            state.sidebar.yolo_mode = current.yolo_mode;
        ...
        if last_dashboard_beat.elapsed() >= std::time::Duration::from_millis(DASHBOARD_BEAT_MS) {
            beat_dashboard_meta(started_at_ms);
            last_dashboard_beat = std::time::Instant::now();
        }
