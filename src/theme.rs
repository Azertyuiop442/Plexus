...
mod tests {
    ...
    #[test]
    fn base_background_is_absolute_black() {
        assert_eq!(BG, Color::Rgb(0x00, 0x00, 0x00));
        assert_eq!(effective_bg(), Color::Rgb(0x00, 0x00, 0x00));
    }

    #[test]
    fn base_background_never_uses_a_terminal_palette_color() {
        let p = Palette::dark();
        for (name, color) in [
            ("BG", BG),
            ...
            assert!(
                !matches!(
                    color,
                    Color::Black | Color::Reset | Color::Indexed(_) | Color::DarkGray
                ),
                "{name} must be absolute RGB black: palette colors follow the terminal \
                 theme (Ghostty Arthur renders ANSI 0 as #3d352a, a visible gray)"
            );
        }
    }
}
