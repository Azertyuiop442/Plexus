...
impl MuxPane {
    ...
    fn agent_status_running(&self) -> (Option<bool>, Option<String>) {
        ...
        if let Some(p) = tty_candidate.as_deref() {
            if let Some(raw) = std::fs::read_to_string(p).ok() {
                if let Some((running, error, updated_at)) = Self::parse_agent_status(&raw) {
                    let mine = self
                        .agent_status_belongs_to_pane(updated_at, crate::dashboard_meta::now_ms());
                    if mine {
        ...
        if let Some(raw) = raw {
            if let Some((running, error, _)) = Self::parse_agent_status(&raw) {
                return (running, error);
            }
        }
    ...
    fn parse_agent_status(raw: &str) -> Option<(Option<bool>, Option<String>, u64)> {
    ...
    fn agent_status_belongs_to_pane(&self, updated_at: u64, now_ms: u64) -> bool {
        updated_at >= self.state.started_at_ms
            && now_ms.saturating_sub(updated_at) <= AGENT_STATUS_MAX_AGE_MS
    }

    pub(crate) fn discover_session_id(&mut self) {
        ...
        for path in candidates {
            ...
            let now_ms = std::time::SystemTime::now()
                ...
                .unwrap_or_default()
                .as_millis() as u64;
            if self.agent_status_belongs_to_pane(updated_at, now_ms) && !sid.is_empty() {
    ...
    pub fn is_session_live(&self, session_id: &str) -> bool {
        ...
        let now_ms = std::time::SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64;
        self.agent_status_belongs_to_pane(updated_at, now_ms)
    }
