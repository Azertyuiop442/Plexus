...
mod tests {
    ...
    fn status_dir(name: &str) -> std::path::PathBuf {
    ...
    fn write_status(dir: &std::path::Path, tty: &str, session: &str, updated_at: u64) {
        std::fs::write(
            dir.join(format!("agent_status-{tty}.json")),
            format!(
                r#"{{"running":true,"sessionId":"{session}","updatedAt":{updated_at}}}"#
            ),
        )
        .unwrap();
    }

    fn spawn_pane_with_status_dir() -> Option<(std::sync::Arc<std::sync::Mutex<MuxPane>>, String, u64)> {
        let Ok((pane, _reader)) = MuxPane::spawn("sleep 1", 80, 24) else {
            return None;
        };
        let (tty, started_at) = {
            let p = pane.lock().unwrap_or_else(|e| e.into_inner());
            (p.state.tty_name.clone(), p.state.started_at_ms)
        };
        if tty.is_empty() {
            return None;
        }
        Some((pane, tty, started_at))
    }

    #[test]
    fn a_fresh_pane_never_adopts_a_previous_pane_session_id() {
        let _lock = crate::ipc::HOME_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = status_dir("adopt");
        let _env = SidebarDirEnv::set(&dir);
        let Some((pane, tty, started_at)) = spawn_pane_with_status_dir() else {
            return;
        };

        write_status(&dir, &tty, "previous-session", started_at.saturating_sub(1_500));
        {
            let mut p = pane.lock().unwrap_or_else(|e| e.into_inner());
            p.discover_session_id();
            assert_eq!(
                p.state.session_id, None,
                "a status written before the pane spawned belongs to another process"
            );
        }

        write_status(&dir, &tty, "own-session", crate::dashboard_meta::now_ms());
        {
            let mut p = pane.lock().unwrap_or_else(|e| e.into_inner());
            p.discover_session_id();
            assert_eq!(p.state.session_id.as_deref(), Some("own-session"));
        }
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn a_session_is_not_live_from_a_pre_spawn_status_file() {
        let _lock = crate::ipc::HOME_LOCK.lock().unwrap_or_else(|e| e.into_inner());
        let dir = status_dir("live");
        let _env = SidebarDirEnv::set(&dir);
        let Some((pane, tty, started_at)) = spawn_pane_with_status_dir() else {
            return;
        };
        ...
        write_status(&dir, &tty, "sess-A", started_at.saturating_sub(2_000));
        assert!(
            !pane.lock().unwrap_or_else(|e| e.into_inner()).is_session_live("sess-A"),
            "a stale status file must not keep a session marked live"
        );

        write_status(&dir, &tty, "sess-A", crate::dashboard_meta::now_ms());
        assert!(pane.lock().unwrap_or_else(|e| e.into_inner()).is_session_live("sess-A"));
        let _ = std::fs::remove_dir_all(&dir);
    }
}
