...
fn bridge_payload_is_fresh(
    ...
    if seq > last_seq || (updated_at > 0 && updated_at > last_updated_at) {
        return true;
    }
    seq == 0 && updated_at == 0
}
...
fn guarded_parse(guard_key: &str, raw: &str) -> Option<ModData> {
    ...
    BRIDGE_GUARDS.with(|g| {
        ...
        match parsed {
            Some(data)
                if bridge_payload_is_fresh(
                    seq,
                    updated_at,
                    guard.last_seq,
                    guard.last_updated_at,
...
mod tests {
    ...
    #[test]
    fn guarded_parse_accepts_updates_from_unversioned_payloads() {
        let first = r#"{ "segments": [ { "text": "alpha" } ] }"#;
        let data = guarded_parse("unversioned", first).expect("first payload parses");
        assert_eq!(data.segments[0].text, "alpha");

        let second = r#"{ "segments": [ { "text": "beta" } ] }"#;
        let data2 =
            guarded_parse("unversioned", second).expect("a payload without seq or updatedAt updates");
        assert_eq!(
            data2.segments[0].text, "beta",
            "a payload carrying no ordering info must not freeze its first value"
        );
    }
