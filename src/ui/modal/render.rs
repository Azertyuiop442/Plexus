...
pub const MIN_MODAL_HEIGHT: u16 = 18;
...
pub fn modal_rect(area: Rect, rows: usize, cmds: usize, content_width: u16) -> Option<Rect> {
    ...
    let natural_h = rows as u16 + 7 + cmds as u16 + 1;
    let height = natural_h.max(MIN_MODAL_HEIGHT).clamp(4, avail_h.max(4));
...
pub fn render_modal(frame: &mut Frame, area: Rect, modal: &Modal, p: &Palette) {
    ...
    let row_max = modal
        ...
        .max()
        .unwrap_or(56);
    let content_width = row_max.max(tabs_width.min(72)).clamp(56, 72);
    ...
    if has_steps {
        ...
        if sum_natural_w <= total_w && n_steps > 0 {
            let extra = total_w.saturating_sub(sum_natural_w);
            let extra_per_tab = extra / n_steps;
            let remainder = extra % n_steps;

            for (idx, step) in modal.steps.iter().enumerate() {
                let seg_w = tab_natural_widths[idx]
                    + extra_per_tab
                    + if idx == n_steps - 1 { remainder } else { 0 };
...
mod tests {
    ...
    #[test]
    fn modal_height_respects_minimum_on_small_rows() {
        let area = Rect::new(0, 0, 140, 40);
        let popup = modal_rect(area, 1, 0, 56).unwrap();
        assert_eq!(popup.height, MIN_MODAL_HEIGHT);
        let tiny_area = Rect::new(0, 0, 80, 12);
        let popup_tiny = modal_rect(tiny_area, 1, 0, 56).unwrap();
        assert_eq!(popup_tiny.height, 10);
    }
}
