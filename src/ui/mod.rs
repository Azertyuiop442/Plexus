...
pub fn render(frame: &mut Frame, state: &mut AppState) {
    ...
    } else if !state.sidebar_open && sidebar_area.width > 0 && area.height > 0 {
        sidebar::render_collapsed_sidebar(frame, sidebar_area);
    }

    let (right_panel, main_area) = if state.panel_maximized {
        (Rect::default(), main_area)
    } else if state.panel_active && state.panel_sidebar_open && main_area.width > state.panel_sidebar_w + 8 {
        ...
        (right, main)
    } else if state.panel_active && !state.panel_sidebar_open && main_area.width > 2 {
        let main = Rect::new(
            main_area.x,
            main_area.y,
            main_area.width.saturating_sub(1),
            main_area.height,
        );
        let right = Rect::new(
            main_area.x + main.width,
    ...
    if state.panel_active && state.panel_sidebar_open && !right_panel.is_empty() {
        if let Some(panel) = panel {
            state.panel_state.reconcile(&panel);
    ...
    } else if state.panel_active && !state.panel_sidebar_open && right_panel.width > 0 && area.height > 0 {
        sidebar::render_collapsed_sidebar(frame, right_panel);
    }
    ...
    let keep_segments = bridge_stale && !state.mods_data.segments().is_empty();
