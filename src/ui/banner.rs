...
pub fn ensure_boot_info(pane: &mut MuxPane, area: Rect) -> bool {
    ...
    let cfg_model = if models.is_none() {
        ...
        let cfg_path = home.join(".commandcode/config.json");
    ...
    let home = crate::ipc::home_dir().to_string_lossy().to_string();
...
fn line_is_menu_hint(line: &str) -> bool {
    let t = line.trim().to_lowercase();
    if t.is_empty() || t.chars().count() > 100 {
        return false;
    }
    let has_esc = t.contains("esc to cancel")
        || t.contains("esc to go back")
        || t.contains("esc to clear")
        || t.contains("esc to exit")
        || t.contains("esc to close");
    let has_select = t.contains("enter to select")
        || t.contains("to select")
        || t.contains("quick select");
    let has_nav = t.contains("navigate")
        || t.contains("↑/↓")
        || t.contains("↑↓")
        || t.contains("arrow keys");
    if has_esc && (has_select || has_nav) {
        return true;
    }
    t.contains("select model")
        || t.contains("switch between the available models")
        || t.contains("select reasoning effort")
...
fn is_terminal_in_interactive_menu(pane: &MuxPane) -> bool {
    ...
    if lines.iter().any(|l| line_is_menu_hint(l)) {
        return true;
    }
...
mod tests {
    ...
    #[test]
    fn line_is_menu_hint_matches_cc_menus() {
        assert!(line_is_menu_hint(
        ...
        assert!(line_is_menu_hint(
            "  1-9 quick select | n notes | c chat | Esc to cancel  "
        ));
        assert!(line_is_menu_hint("  Select model  "));
        assert!(line_is_menu_hint(
            "  Select reasoning effort for z-ai/glm-5.3-flash  "
        ));
        assert!(!line_is_menu_hint(""));
        assert!(!line_is_menu_hint("  ask your question  "));
        assert!(!line_is_menu_hint("  ❯ "));
        assert!(!line_is_menu_hint("  ? for shortcuts  "));
    }

    #[test]
    fn banner_hides_for_cc_interactive_menu() {
        use crate::ui::pane::MuxPane;
        use ratatui::layout::Rect;

        let Ok((pane, _reader)) = MuxPane::spawn("sleep 1", 120, 30) else {
            return;
        };
        let mut p = pane.lock().unwrap_or_else(|e| e.into_inner());

        p.feed("  type to search · ↑/↓ navigate · shift+↑/↓ jump provider · enter to select · esc to cancel  \r\n".as_bytes());
        p.feed("  ↑/↓ navigate · enter to select · esc to go back  \r\n".as_bytes());

        let area = Rect::new(0, 0, 120, 30);
        assert_eq!(
            banner_height(&mut p, area),
            0,
            "banner must hide while a CC interactive menu is on screen"
        );
    }

    #[test]
    fn banner_returns_after_menu_dismissed() {
        use crate::ui::pane::MuxPane;
        use ratatui::layout::Rect;
        ...
        p.feed("  type to search · ↑/↓ navigate · enter to select · esc to cancel  \r\n".as_bytes());
        assert_eq!(banner_height(&mut p, area), 0);

        for _ in 0..40 {
            p.feed("plain output line\r\n".as_bytes());
        }
        p.feed("  ask your question  \r\n".as_bytes());
        assert!(
            banner_height(&mut p, area) > 0,
            "banner must return once the menu is gone"
        );
    }
}
